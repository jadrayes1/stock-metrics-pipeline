...
// comment for the full verified-live rationale (BRK.A, ELF). Deliberately
// left preferring the cumulative fact on a tie (never changed to prefer
// standalone) -- an earlier attempt at that reversal, tested against
// AAPL's 15+ years of real historic facts as a regression check, corrupted
// fiscal-Q4 derivation for 2009-2020 (a double-counting artifact from how
// AAPL's own historic XBRL tagging ambiguity interacts with a changed
// default). See pickStandaloneDurationFact below for how the SPCX-style
// "no earlier quarter to anchor against" case is handled instead, without
// touching this proven, already-correct default at all.
...
// no earlier quarter to anchor a subtraction against (see its own use of
// this below). Restricted to genuinely short (<=120 day) durations, so it
// can never mistake a cumulative fact for a standalone one. Verified live
// for SPCX (SpaceX): its only 10-Q on file is a Q2, with no Q1 filed
// (its first-ever quarterly SEC report), yet the SAME 10-Q also tags a
// real, directly-disclosed 3-month net income fact alongside the 6-month
// cumulative one pickDurationFact prefers by default -- this recovers it
// without needing an anchor to subtract against, and without changing
// what gets published for any ticker/year that already has one.
...
// See pickStandaloneDurationFact's own comment for the rationale.
function findStandaloneSecValueForFyFp(gaapFacts, concepts, fy, fp, unit) {
  for (const concept of concepts) {
    const facts = gaapFacts[concept]?.units?.[unit] || [];
    const fact = pickStandaloneDurationFact(facts, fy, fp);
    if (fact) return { value: fact.val, concept: `us-gaap_${concept}`, start: fact.start, end: fact.end };
  }
  return null;
}
...
function buildSecSyntheticPfcfReports(gaapFacts, cik) {
  ...
  for (const { fy, fp } of fyFpKeys.values()) {
    ...
    // additive alongside icItems/cfItems above (which are completely
    // unchanged), only ever consulted by decumulateYtdByYear's own
    // orphan-quarter fallback when there's no earlier quarter to anchor a
    // normal cumulative subtraction against.
    const standaloneOcfFact = findStandaloneSecValueForFyFp(gaapFacts, SEC_OCF_CONCEPTS, fy, fp, 'USD');
    const standaloneNetIncomeFact = findStandaloneSecValueForFyFp(gaapFacts, SEC_NET_INCOME_CONCEPTS, fy, fp, 'USD');
    const standaloneCf = [standaloneOcfFact].filter(Boolean).map((r) => ({ concept: r.concept, label: `${r.concept} (SEC XBRL enrichment, standalone)`, value: r.value }));
    const standaloneIc = [standaloneNetIncomeFact].filter(Boolean).map((r) => ({ concept: r.concept, label: `${r.concept} (SEC XBRL enrichment, standalone)`, value: r.value }));

    const entry = { cik: normalizedCik, form: 'SEC-XBRL', endDate, report: { ic: icItems, cf: cfItems, standaloneIc, standaloneCf } };
...
// up near pickStandaloneDurationFact, which needs it earlier in the file.)
  ...
  for (const [yearStr, q] of Object.entries(ytdByYear)) {
    ...
    if (q[2] != null) {
      if (standaloneFlags[2]) {
        ...
        // Deliberately NOT setting cumulativeThrough[2] here (see Q3's own
        // identical comment below) -- a company can genuinely have been
        // operating, with a real Q1 net income that simply predates its
        // SEC-reporting obligation (verified live: SPCX/SpaceX's actual
        // first-ever 10-Q is a Q2) -- treating "just Q2" as "cumulative
        // since fiscal year start" would be WRONG once Q3 shows up as a
        // genuinely cumulative fact, silently understating Q3 by whatever
        // the real (unknown to us) Q1 was. Publish this quarter's own real
        // value and stop -- never let an admittedly-incomplete chain
        // propagate into a later quarter's arithmetic.
      } else {
    ...
    if (q[3] != null) {
      if (standaloneFlags[3]) {
        ...
        // Deliberately NOT setting cumulativeThrough[3] here -- it isn't a
        // real cumulative-through-Q3 total, and Q4's own derivation below
        // (annualByYear[year] - cumulativeThrough[3]) needs a genuine one
        // or not at all; better to leave Q4 ungated by this than derive it
        // from a value that was never really cumulative.
      } else {
