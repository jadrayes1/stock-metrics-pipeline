...
// A fresh reconstruction attempt can come back empty, narrower, or (as
// verified live this session for a sibling pipeline's identical function)
// subtly WRONG on a day where extraction/reconciliation hits an edge
// case for this specific ticker. This used to replace the ENTIRE existing
// array the moment fresh contained even one label the existing one
// didn't, on the assumption that fresh is normally a strict superset —
// verified that assumption doesn't always hold: a single genuinely new
// label can ride alongside several OTHER, independently wrong values for
// labels that were already correctly published, and the old logic would
// swap in the whole array, silently overwriting real data with wrong
// data despite the stated "never regress to narrower" guarantee. This is
// a real union instead: every existing point is kept forever (never
// dropped, never overwritten by a fresh value for the same label), fresh
// only ever contributes labels that aren't already published, and the
// merged result is re-sorted chronologically (a genuinely new label can
// be OLDER than some already-published ones — gap-backfill fixes
// routinely recover previously-missing older quarters, not just newer
// ones) and re-capped at the same QUARTERS_OF_HISTORY rolling window the
